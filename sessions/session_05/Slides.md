...
<iframe
    width="900"
    height="500"
    src="http://209.38.211.172/release_activity_weekly.svg"
    frameborder="0"
    allowfullscreen
></iframe>
...
<iframe
    width="900"
    height="500"
    src="http://209.38.211.172/commit_activity_weekly.svg"
    frameborder="0"
    allowfullscreen
></iframe>
...
The TAs will start the simulator **today** in the end of the exercise session (in between 11:00 and 12:00).
...
### Pull-requests to `repositories.py` have all conflicts


- Currently, I do not know how many groups have sent their two URLs:
...
- We have to fix that file at the beginning of the exercise session today!
- I do not know how to merge the remaining pull-requests.
- Before sending a pull-request, make sure that you send it against the most current version of `repositories.py`. Otherwise, things get messy and we get confused!
...
- Please fix and complete [the groups registration file](https://ituniversity.sharepoint.com/:x:/r/sites/2026BScDevOpsSoftwareEvolutionandSoftwareMaintenancecopy/Shared%20Documents/General/Groups.xlsx?d=w68b4499a92ef4448903040a08bbce6f5&csf=1&web=1&e=uYgFbC)
...
Jan's guest lecture material is shared with you on [Teams]()

Please do not share it on other services!
Jan and Eficode wish to share this material with you but not publicly with other parties.
In case you want to share the material with others, e.g., at work, ask Jan for permission.

---

### Jan talked psychological safety... But how to do it?
...
---
  ...
  * I will not give you an introduction to Ansible, some good introductory resources are:
    ...
    - _"Getting Started with Ansible"_ section in chapter one of [_DevOps for the Desperate_](https://nostarch.com/devops-desperate).
...
Two weeks ago, I mentioned that when creating (provisioning) a VM with Vagrant, Vagrant runs one or more `provision` scripts after a VM is created.
This `provision` step is usually called configuration management.

We focus on configuration management with shell scripts.
However, [Vagrant](https://www.vagrantup.com/docs/provisioning/) there are other configuration management tools, are supported too:

  * [Ansible Playbooks](https://docs.ansible.com/projects/ansible/latest/playbook_guide/playbooks.html)
  * [Chef cookbooks](https://docs.chef.io/chef_solo/)
...
echo "deb [ arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb-server-8.0.gpg ] https://repo.mongodb.org/apt/ubuntu jammy/mongodb-org/8.0 multiverse" | \\
    sudo tee /etc/apt/sources.list.d/mongodb-org-8.0.list
if ! grep -qF "jammy/mongodb-org/8.0" /etc/apt/sources.list.d/mongodb-org-8.0.list; then
  echo "deb [ arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb-server-8.0.gpg ] https://repo.mongodb.org/apt/ubuntu jammy/mongodb-org/8.0 multiverse" | \\
    sudo tee /etc/apt/sources.list.d/mongodb-org-8.0.list
fi
cat /etc/apt/sources.list.d/mongodb-org-8.0.list
